...
@interface XSAlertManagerViewDelegate : NSObject<UIAlertViewDelegate>

@property (nonatomic,strong) void(^completion)(NSInteger);

+(void)addDelegate:(XSAlertManagerViewDelegate*)delegate;
+(void)removeDelegate:(XSAlertManagerViewDelegate*)delegate;

@end

@implementation XSAlertManager
...
+ (void)ask:(void(^)(NSString * answer))block title:(NSString*)title withTextPrompt: (NSString *) prompt{
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
        ...
        XSAlertManagerViewDelegate *delegate = nil;
        if (block) {
            delegate = [[XSAlertManagerViewDelegate alloc] init];
            delegate.completion = ^(NSInteger index){
                if (index == 1) {
                    block(tf.text);
                }
            };
            [XSAlertManagerViewDelegate addDelegate:delegate];
        }
...
@implementation XSAlertManagerViewDelegate
